...
(function() {
  var CoffeeScript, DIRECTIVE, DepGraph, EXPLICIT_PATH, HEADER, HoldingQueue, Snockets, async, compilers, fs, jsExts, minify, parseDirectives, path, stripExt, timeEq, uglify, _,
    __slice = [].slice,
  ...
  module.exports = Snockets = (function() {
    ...
    Snockets.prototype.updateDirectives = function() {
      ...
      requireTree = function(dirName, done) {
        return _this.readdir(_this.absPath(dirName), flags, function(err, items) {
          if (err) {
            return done(err);
          }
          items = items.sort();
          return async.forEachSeries(items, function(item, next) {
            var itemPath;
            ...
            return _this.stat(_this.absPath(itemPath), flags, function(err, stats) {
              ...
              if (stats.isFile()) {
                return q.task(itemPath, next);
              } else {
                return requireTree(itemPath, next);
              }
            });
          }, function(err) {
            if (err) {
      ...
      return this.readFile(filePath, flags, function(err, fileChanged) {
        ...
        directives = parseDirectives(_this.cache[filePath].data.toString('utf8'));
        return async.forEachSeries(directives, function(directive, next) {
          var command, relPaths, words;
          words = directive.replace(/['"]/g, '').split(/\s+/);
          command = words[0], relPaths = 2 <= words.length ? __slice.call(words, 1) : [];
          switch (command) {
            case 'require':
              return async.forEachSeries(relPaths, function(relPath, next) {
                return require(relPath, next);
              }, next);
            case 'require_tree':
              return async.forEachSeries(relPaths, function(relPath, next) {
                return requireTree(_this.joinPath(path.dirname(filePath), relPath), next);
              }, next);
          }
        }, function(err) {
          if (err) {
